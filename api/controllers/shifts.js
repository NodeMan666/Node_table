...
import {publishMailerHandler} from '../mailers/publish_mailer';
...
async function publishByShiftId(req, res, next) {
    const {orgId, shiftId} = req.params;
    const shift = await Shift.findOne({_id: shiftId, organization: orgId});
    const shiftRequests = await ShiftRequest.find({organization: orgId, shift: shiftId}).populate('user').exec();

    for(const shiftRequest of shiftRequests) {
        if(shiftRequest.user) {
            await publishMailerHandler(shiftRequest.user, shift, 'localtable-welcome', 'Welcome to LocalTable');
        }
    }
